...
def process_monthly_dependability(input_excel, months):
    for month in months:
        source_sheet = month
        target_pct = 75.0
        output_excel = f"{month}.xlsx"
        summary_sheet_name = f"75_Percent_Dependibility_{month}"

        # If output file exists, remove old sheets (but since it's new per month, we'll create fresh)
        if os.path.exists(output_excel):
            os.remove(output_excel)  # Start fresh for each run

        # STEP 1: Read base sheet from input file and compute dependability per junction
        ...
        # Write junction sheets to NEW output file
        with pd.ExcelWriter(output_excel, engine="openpyxl") as writer:
            for sheet_name, frame in junction_frames.items():
                frame.to_excel(writer, sheet_name=sheet_name, index=False)

        # STEP 2: Re-open OUTPUT workbook and find best rows, highlight, collect summary
        wb = load_workbook(output_excel)
        highlight_fill = PatternFill(start_color="FFFF00", end_color="FFFF00", fill_type="solid")
        summary_data = []

        print(f"\n=== Processing Sheets for {month} ===")
        ...
        # STEP 3: Create summary sheet in the OUTPUT file
        if summary_data:
            # Create summary sheet
            summary_ws = wb.create_sheet(title=summary_sheet_name, index=0)  # Put at front

            # Write header (Junctions, Year, Dependability (%), Value (MCM))
            ...
            col_widths = [30, 12, 18, 15]
            for i, w in enumerate(col_widths, start=1):
                summary_ws.column_dimensions[summary_ws.cell(1, i).column_letter].width = w

            print(f"\n📘 Summary sheet '{summary_sheet_name}' created with {len(summary_data)} rows in {output_excel}.")

        else:
            print(f"\n⚠️ No summary data collected for {month}; summary sheet will not be created.")

        # Save output workbook
        wb.save(output_excel)

        # Assuming you have this function to autofit columns
        # from excel import autofit_columns
        # autofit_columns(output_excel)

        print(f"\n✅ Finished processing {month}. Output saved to {output_excel}.")

# === Example usage ===
INPUT_PATH = r"C:\Users\Swapnali\Desktop\Sakina Maam Work\Dependibility\input.xlsx"  # Replace with your input file path
MONTHS = ["Jun", "Jul", "Aug", "Sept"]
process_monthly_dependability(INPUT_PATH, MONTHS)